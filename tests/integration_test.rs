...
use chrono::prelude::*;
extern crate chrono;
use rawsort::executor::{Executor, ExecutionPlan};
...
fn fixture(f: &'static str) -> String{
    Path::new(file!()).parent().unwrap().join("fixtures").join(f).to_str().unwrap().to_string()
}
fn moves(strs: &[(&'static str, &'static str)]) -> Vec<(PathBuf,PathBuf)>{
    strs.to_vec().iter().map(|(s1, s2)| (PathBuf::from(s1), PathBuf::from(s2))).collect()
}
...
fn it_detects_bad_formats() {
    ...
    assert_eq!(plan.moves, moves(&[("tests/fixtures/simple/20171104-DSC_1233.NEF", "")]));
...
fn it_will_not_format_almost_a_good_format() {
    ...
    assert_eq!(plan.moves, moves(&[("tests/fixtures/simple/20171104-DSC_1233.NEF", "year")]));
...
fn it_will_format_a_good_format() {
    ...
    assert_eq!(plan.moves, moves(&[("tests/fixtures/simple/20171104-DSC_1233.NEF", "2017")]));
}

#[test]
fn it_detects_potential_overwrites() {
    let (plan, exec) = make_plan("multiple", "[year]");
    assert_eq!(exec.validate(&plan).unwrap_err(),"Source and target file counts aren't equal: 2 file(s) for source, 1 file(s) for target.");
}

#[test]
fn it_plans_properly() {
    let (plan, exec) = make_plan("multiple", "[year]/[month]/[day]/[hour]/[minute]/[second]/[filename].[ext]");
    assert_eq!(plan.moves, moves(&[("tests/fixtures/multiple/20171104-DSC_1236.JPG", "2017/11/4/12/45/23/20171104-DSC_1236.JPG.JPG"), ("tests/fixtures/multiple/20171104-DSC_1237.JPG", "2017/11/4/12/45/34/20171104-DSC_1237.JPG.JPG")]));
    assert_eq!(plan.dirs_to_create, dirs(&["2017/11/4/12/45/23", "2017/11/4/12/45/34"]));
}